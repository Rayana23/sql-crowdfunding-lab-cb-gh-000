...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT users.name, users.age, SUM(pledges.amount) AS total_pledged FROM users
...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, SUM(pledges.amount)-projects.funding_goal AS over FROM projects
  INNER JOIN pledges ON pledges.project_id = projects.id
  GROUP BY projects.id
